...
def main():
    ...
    os.chdir("/home/yannick/FRI/advanced-graph-kernels/optimization")
    ...
    datasets = ["AIDS", "Mutagenicity", "NCI1", "NCI109",
                "PROTEINS", "BZR", "COX2", "DHFR",
                "MUTAG", "PTC_FM", "PTC_FR", "PTC_MM",
    ...
    name = f"testparalel_{model.__name__}_{dataset}_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}_log.txt"

    with open(f"testparalel/{name}", "w") as f:
        for s in solution:
